const getNavigationData = async () => {
  ...
  return responseJson?.data?.menus?.nodes;
}
...
const Header = async () => {
  ...
  const renderNavigation = (mobile) => {
    const {menuItems} = navData[0];
    return menuItems && menuItems?.edges?.map((menu_item)=> {
      const {label, uri, id} = menu_item?.node;
      const menuNode = mobile ? ( <a key={id} href={uri} className="-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-gray-900 hover:bg-gray-50">{label}</a>) :(<a  href={uri} key={id}className="text-sm font-semibold leading-6 text-gray-900">{label}</a>);
      return menuNode;
    })
  }
